...
def str_copy_paste(htl, strpath, sheet,str_file):
    ...
    for file in os.listdir(strpath):
        if file.__contains__(htl) == True:
            ...
            df_trans1["Hotel LY Occ"] = df_trans1["Hotel TY Occ"]/(1+(df_trans1["Hotel Occ % Change"]/100)).where(1+(df_trans1["Hotel Occ % Change"]/100) != 0, 1)
            df_trans1["Hotel LY ADR"] = df_trans1["Hotel TY ADR"] / (1 + (df_trans1["Hotel ADR % Change"] / 100)).where(1 + (df_trans1["Hotel ADR % Change"] / 100) !=0,1)
            df_trans1["Comp LY Occ"] = df_trans1["Comp TY Occ"] / (1 + (df_trans1["Comp Occ % Change"] / 100)).where(1 + (df_trans1["Comp Occ % Change"] / 100)!=0,1)
            df_trans1["Comp LY ADR"] = df_trans1["Comp TY ADR"] / (1 + (df_trans1["Comp ADR % Change"] / 100)).where(1 + (df_trans1["Comp ADR % Change"] / 100)!=0,1)
            ...
            shutil.move(strpath + "//"+ file, strpath + "/archive/"+ file)
...
if __name__ =='__main__':
    ...
    str_copy_paste(htl, str_path, sheet,str_file)